 ...
 */
class JsonRpcServiceBaseTest extends WordSpec with BeforeAndAfter with MockitoSugar with Matchers {
  ...
  "prepareConnection" should {

    "throw exception when connection is null" in {
      a[NullPointerException] should be thrownBy {
        JsonRpcServiceBase.prepareConnection( null )
      }
    }

    "set connection request method to POST" in {
    ...
    "throw Runtime Exception if request method is not supported" in {
  ...
  "getConnectionStream" should {

    "throw RuntimeException when HttpURLConnection is null" in {
      a[NullPointerException] should be thrownBy {
        JsonRpcServiceBase.getConnectionStream( null )
      }
    }

    "return the connection InputStream" in {
    ...
    "return the connection ErrorStream when Response Code is not 200" in {
  ...
  "extractErrorResponse" should {

    "throw exception when JsonObject is null" in {
      a[NullPointerException] should be thrownBy {
        JsonRpcServiceBase.extractErrorResponse( null )
      }
    }

    "return an JsonRpcException" in {
      JsonRpcServiceBase.extractErrorResponse( mockJsonObject ) shouldBe a[JsonRpcException]
    }

    "extract from { error: { message: \"anErrorMessage\" } }" in {
      JsonRpcServiceBase.extractErrorResponse( mockJsonObject ).getMessage shouldBe "anErrorMessage"
    }
  }

  "getExceptionForIOException" should {

    "never throw exception when IOException is null" in {
    ...
    "return JsonRpcException when IOException is null" in {
      JsonRpcServiceBase.getExceptionForIOException( null ) shouldBe a[JsonRpcException]
    }

    "return default JsonRpcException when IOException is null" in {
      JsonRpcServiceBase.getExceptionForIOException( null ).getMessage shouldBe "JSON-RPC exception"
    }

    "return wrapped IOException" in {
      JsonRpcServiceBase.getExceptionForIOException( new IOException ).getCause shouldBe a[IOException]
    }
  }

  "convertStreamToString" should {
    "throw exception when InputStream is null" in {
      a[NullPointerException] should be thrownBy {
        JsonRpcServiceBase.convertStreamToString( null )
      }
    }

    "return \"(empty)\" when InputStream is not null" in {
